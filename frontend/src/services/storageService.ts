...
export const getSession = (): GameSession | null => readJson<GameSession | null>(SESSION_KEY, null)
...
export const getCompetitionSettings = (): CompetitionSettings =>
  readJson<CompetitionSettings>(COMPETITION_SETTINGS_KEY, DEFAULT_COMPETITION_SETTINGS)

export const saveCompetitionSettings = (settings: CompetitionSettings) => {
  window.localStorage.setItem(COMPETITION_SETTINGS_KEY, JSON.stringify(settings))
}
...
export const setQuestionOverride = (key: string, active: boolean) => {
  ...
  window.localStorage.setItem(QUESTION_OVERRIDES_KEY, JSON.stringify(overrides))
}
...
export const getLeaderboard = (): LeaderboardEntry[] => readJson<LeaderboardEntry[]>(LEADERBOARD_KEY, [])

export const saveAdminState = (state: Record<string, unknown>) => {
  window.localStorage.setItem(ADMIN_KEY, JSON.stringify(state))
}

export const getAdminState = (): Record<string, unknown> => readJson<Record<string, unknown>>(ADMIN_KEY, {})
...
export const getWinner = (): Winner | null => readJson<Winner | null>(WINNER_KEY, null)
...
export const getCustomQuestions = <T,>(level: number): Record<string, T> =>
  readJson<Record<string, Record<string, T>>>(CUSTOM_QUESTIONS_KEY, {})[String(level)] ?? {}

export const saveCustomQuestion = <T extends { id: string }>(level: number, item: T) => {
  const all = readJson<Record<string, Record<string, T>>>(CUSTOM_QUESTIONS_KEY, {})
...
export const deleteCustomQuestion = (level: number, id: string) => {
  const all = readJson<Record<string, Record<string, unknown>>>(CUSTOM_QUESTIONS_KEY, {})
  if (all[String(level)]) {
...
export const getQuestionEdits = <T,>(level: number): Record<string, Partial<T>> =>
  readJson<Record<string, Record<string, Partial<T>>>>(QUESTION_EDITS_KEY, {})[String(level)] ?? {}

export const saveQuestionEdit = <T,>(level: number, id: string, patch: Partial<T>) => {
  const all = readJson<Record<string, Record<string, Partial<T>>>>(QUESTION_EDITS_KEY, {})
  all[String(level)] = { ...(all[String(level)] ?? {}), [id]: { ...(all[String(level)]?.[id] ?? {}), ...patch } }
  window.localStorage.setItem(QUESTION_EDITS_KEY, JSON.stringify(all))
}

export const getQuizSetOverrides = (): QuizSetOverrides => readJson<QuizSetOverrides>(QUIZSET_OVERRIDES_KEY, {})

export const setQuizSetOverride = (setId: string, levelKey: keyof QuizSetOverrides[string], questionId: string) => {
...
export const getQuestionStats = (): Record<string, QuestionStat> => readJson<Record<string, QuestionStat>>(QUESTION_STATS_KEY, {})

export const recordQuestionStat = (level: number, id: string, outcome: 'shown' | 'correct' | 'incorrect') => {
