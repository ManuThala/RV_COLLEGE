...
function LeaderboardPage() {
    ...
    const loadLeaderboard = async () => {
      try {
        ...
        if (!cancelled && result.success && result.entries) {
          setRemoteLeaderboard(result.entries);
        }
      } catch {
        // Local leaderboard remains the fallback.
      }
    };

    void loadLeaderboard();
    const refreshTimer = window.setInterval(loadLeaderboard, 3000);
  ...
  const leaderboard = useMemo(
    () =>
      (remoteLeaderboard ?? getLeaderboard()).sort((a, b) => {
              ...
              <table className="w-full min-w-[1100px] border-collapse text-sm">
                ...
                <tbody>
                  {filtered.map((entry, index) => (
                    <tr
                      key={entry.id}
                      className="border-t border-slate-800 bg-slate-900/40 hover:bg-slate-800/40"
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span
                          className={`inline-block rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.1em] ${statusBadgeClass(entry.status)}`}
                        >
                          {entry.status}
                        </span>
                      </td>
