import { ArrowLeft, AlertTriangle, ShieldCheck, TimerReset } from 'lucide-react'
      ...
      <div className="mx-auto max-w-4xl">
        ...
        <div className="glass-panel rounded-3xl p-8 md:p-10">
          ...
          <div className="space-y-4">
            {levels.map((level) => (
              <div key={level} className="rounded-2xl border border-slate-700 bg-slate-800/60 p-4 text-slate-200">
                {level}
              </div>
            ))}
          </div>
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5">
              ...
              <p className="text-sm text-slate-200">Every incorrect attempt and timeout adds penalty time to the final score.</p>
            </div>
            <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-5">
              ...
              <p className="text-sm text-slate-200">The correct incident-response sequence must meet the required accuracy threshold to pass.</p>
            </div>
          </div>

          <div className="mt-8 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-4 text-sm text-cyan-100">
            Demo version: Data is stored on this device. Multi-device real-time competition requires backend integration.
          </div>
